    ...
    def clicLoginBtn(self):
        def dialog_window():
            def clickChangeUser():
                config = configparser.ConfigParser()
                config.read(CONFIG_PATH)
                if self.comboBox.currentIndex() == 0 and config.get("Login", "user") == "1":
                    config.set("Login", "user", "0")  # 0 - налоговая 1 - юр лицо
                    ...
                    self.load_configs()
                elif self.comboBox.currentIndex() == 1 and config.get("Login", "user") == "0":
                    config.set("Login", "user", "1")  # 0 - налоговая 1 - юр лицо
                    with open(CONFIG_PATH, "w") as config_file:
                        config.write(config_file)
            ...
            config = configparser.ConfigParser()
            config.read(CONFIG_PATH )
            self.comboBox.setCurrentIndex(int(config.get("Login", "user")))
